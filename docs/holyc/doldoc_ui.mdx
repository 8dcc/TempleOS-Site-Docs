...
```holyc
 ...
 ```
 ### User interfaces
You may want to create user interfaces using DolDoc. There are numerous ways to do this,one way to do this is to use `DocMenu`. This will return a value as defined by `LE` or `RE`. These mean left expression and right expression. Let's see an example.
```holyc
U0 CreateDialog() {
    CDocEntry *nums[3];
    I64 i;
